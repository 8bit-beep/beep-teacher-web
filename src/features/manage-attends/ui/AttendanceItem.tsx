...
const AttendanceItem = ({ data, roomId }: Props) => {
  ...
  return (
    <div
      className={`w-full h-12 flex items-center px-4 gap-4 ${data.statuses[0].status ? (data.statuses[0].status.name === "외박" ? "bg-red-light" : data.statuses[0].status.name === "외출" ? "bg-green-light" : "bg-static-white") : "bg-greyscale-10"}`}>
